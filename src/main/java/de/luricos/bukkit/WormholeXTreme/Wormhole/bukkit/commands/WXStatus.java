    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        //if (!CommandUtilities.playerCheck(sender) || WXPermissions.checkPermission((Player) sender, PermissionType.CONFIG)) {
            ...
            if ((args[0].equalsIgnoreCase("a")) || (args[0].equalsIgnoreCase("all"))) {
                sender.sendMessage(Messages.createNormalMessage("\u00A76----------------------------"));
                sender.sendMessage(Messages.createNormalMessage("System status"));
                sender.sendMessage(Messages.createNormalMessage("\u00A76----------------------------"));
                sender.sendMessage(Messages.createNormalMessage("DBConnection: " + ((StargateDBManager.isConnected()) ? "\u00A72ready" : "\u00A74failed")));
                sender.sendMessage(Messages.createNormalMessage(""));
            }
        /*} else {
